...
int thrd_func1 (void *p){
	...
	while(!kthread_should_stop()){
		printk(KERN_INFO " kernel function is activated and we are in it");
		msleep(1000);
...
static int __init chr_driver_init(void){
 ...
 /* thread ........*/
	chr_thread = kthread_create(thrd_func1, NULL, "my_device");
	if (chr_thread){
		wake_up_process(chr_thread); // we will start the kernel thread by writing wake up process
	}else{
